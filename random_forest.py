...
def main():
	data=[]
	ensembles=[]
	attributes=read_data("data/handwriting/train.data")
	labels=read_data("data/handwriting/train.labels")
	data=[]
	for i in range(len(attributes)):
		if labels[i]=="1":
			data.append(attributes[i]+labels[i])
		else:
			data.append(attributes[i]+"0")

	labels=[i[-1] for i in data]
	for i in range(5):
		temp=[]
		random_indexes=[random.randint(0,999) for i in range(500)]
		for index in random_indexes:
			temp.append(data[index])

		ensembles.append(create_tree(temp,attributes,labels,gain))

	test_attributes=attributes
	test_class_labels=labels
	test_data=[]
	...
	for i in range(len(ensembles)):
		classification=classify(ensembles[i],test_data)
		positive=0
		negative=0
		...
		training_data.append(classification)
	...
	for i in range(len(training_data[0])):
		...
		svm_training_data.append(temp)


	#_____________________________________________________________________________________
	test_attributes=read_data("data/handwriting/test.data")
	test_labels=read_data("data/handwriting/train.labels")
	test_data=[]
	for i in range(len(test_attributes)):
		if test_labels[i]=="1":
			test_data.append(test_attributes[i]+test_labels[i])
		else:
			test_data.append(test_attributes[i]+'0')
	...
	for i in range(len(ensembles)):
		...
		total=positive+negative
		print "Accuracy of tree",i+1, "on data is",float(positive)/total
		temp_test_data.append(classification)

	svm_test_data=[]
	for i in range(len(temp_test_data[0])):
		temp=[]
		for row in temp_test_data:
			if row[i]=="0":
				temp.append(float("-1"))
	...
	return svm_training_data, svm_test_data
